...
def color_changed(new_intval):
    ...
    #CHANGE COLOR OF CIRCLE (REDRAW CIRCLE)
    newColor = '#' + hexstring(red_intvar) + hexstring(green_intvar) + '00'
    circle_item = canvas.create_oval(100, 100, 250, 250, 
                                 outline='#000000', fill=newColor) #4
...
#New canvas
canvas = Tkinter.Canvas(root, width=300, height=300, background='#FFFFFF') #1
canvas.grid(row=0, column=2) #2
